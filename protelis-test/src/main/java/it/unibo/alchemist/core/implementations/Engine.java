...
public final class Engine<T> implements Simulation<T> {
    ...
    private final List<OutputMonitor<T>> monitors = new LinkedList<>();
    ...
    public Engine(final Environment<T> e, final long maxSteps, final Time t) {
        env = e;
        dg = new MapBasedDependencyGraph<>(env, handlers);
    ...
    public void run() {
        synchronized (env) {
            ...
            } catch (RuntimeException | InterruptedException | Error e) { // NOPMD
                error = e;
            } finally {
