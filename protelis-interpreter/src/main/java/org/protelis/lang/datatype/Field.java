...
/**
 * A field associates a collection of devices with values. One such device is the local one.
 *
...
public interface Field<T> extends Serializable {
     ...
     * value, and whose computation has base as the initial element.
     *
     ...
     * value, and whose computation has base as the initial element.
     * This is the most
     * generic version, actually performing a map plus reduce operation.
     *
     * @param <R>      Return type
     * @param base     base value for the computation
    ...
    /**
     * Reduction operation with base value (fold), which uses the local value as an initial element.
     *
     * @param combiner reduction function takes two R and returns a single R
     * @return the result of the reduction
     */
     ...
     * the initial element.
     * This is the most generic version, actually performing a map plus reduce operation.
     *
     * @param <R>      Return type
     * @param mapper   mapping function for pre-processing field entries
     * @param combiner reduction function takes two R and returns a single R
     ...
     * key, and whose computation has base as the initial element.
     *
     * @param base     base value for the computation
     ...
     * @return a new field with the same values, but with possibly fewer entries.
     *     If the restriction is operated over the same domain of the current field, the field itself can be returned.
     */
     ...
     *         field contained only the local value
     */
    ...
    /**
     * Reduction operation over the field. Ignores the local value and returns an
     * {@link Option}. This is the most general version, operating a map plus reduce
     * operation.
     *
