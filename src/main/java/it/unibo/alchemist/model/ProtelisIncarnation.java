...
ts(A) :- search(A, [[[['service 1', 1.3]], ["service 2", 0]]]).
			...
			try {
				...
				if(s.isSuccess()) {
					...
					if(binds.isEmpty()) {
						...
						final Optional<Var> vv = binds.stream().filter(v -> v.getTerm() instanceof alice.tuprolog.Number).findFirst();
						if(vv.isPresent()) {
	...
	public IMolecule createMolecule(final String s) {
		if(s.startsWith(ProtelisProgram.PROGRAM_ID_PREFIX)) {
			final ProtelisProgram prog = ProtelisProgram.getProgramByID(new FasterString(s));
			return prog;
		}
